...
CLASS_INDEX = None
MODEL_PATH = os.path.join(execution_path, settings.MEDIA_ROOT + "\\" + "idenprof_061-0.7933.h5")
JSON_PATH = os.path.join(execution_path, settings.MEDIA_ROOT + "\\" + "idenprof_model_class.json")
...
def run_inference(filename):
    ...
    picture = settings.MEDIA_ROOT + "\\" + filename  # os.path.join(execution_path, filename)
    ...
    for result in predictiondata:
        ...
        resultDict.update({result[0]: round(result[1] * 100, 2)})
    return resultDict
